...
# %pip install --pre -r requirements_scale_up.txt
```
...
Note that we routinely update the code when improvements are available, so the logs shown here don't necessarily reflect the current behavior of the code.
...
We also see that the IceCube call completed very quickly but required a significant amount of memory, which was the code behavior prior to an update in August 2026.
...
kwargs_dict = {
    ...
    "archives": {
        ...
        "HEASARC": {"catalog_constraints": {"FERMIGTRIG": 1.0, "SAXGRBMGRB": 3.0,
...
```bash
$ yaml_run_id=demo-kwargs-yaml
$ bash code_src/helpers/scale_up.sh -r "$yaml_run_id" -d "use_yaml=true" -a "Gaia HEASARC PanSTARRS WISE ZTF"
```
